...
import { useGetChihois } from "@/services/chihoi-service";
...
export function ChihoisTable<TData, TValue>({ columns, initialData, total }: DataTableProps<TData, TValue>) {
  const { pagination, onPaginationChange } = usePagination();
  const { sorting, onSortingChange, field, order } = useSorting();
  const { columnFilters, onColumnFiltersChange, filterObject } = useColumnsFilter();

  const { data, isFetching } = useGetChihois(initialData, {
